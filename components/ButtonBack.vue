<template>
  <button
    ...
    @click="$router.go(-1)"
  >
    <!-- Temporarliy removing click event to check if using router history globally will suffice -->
    <!-- @click="$emit('click')" -->
    <slot>BACK</slot>
    <span v-if="subtitle"
      ><br />
      <span class="text-xs font-normal">{{ subtitle }}</span></span
    >
    <div
      v-if="showIcon"
      class="absolute top-0 left-0 bottom-0 flex items-center pl-3"
    >
      <svg
    ...
    <div
      ...
      class="absolute top-0 left-0 bottom-0 flex items-center pl-3"
    >
      <img class="w-8" :src="preIcon" aria-hidden="true" />
    </div>
  </button>
</template>
